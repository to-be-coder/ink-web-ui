...
interface FocusItem {
  msgIdx: number
  partIdx?: number  // undefined for user messages
}
...
export function NewAIChatThreadV2() {
  ...
  // Only run spinner when there are active streaming/running parts
  ...
  // Auto-collapse reasoning and tool-call parts
  useEffect(() => {
  ...
  // Track which message each focus item belongs to for separators
  let prevMsgIdx = -1

  return (
    <Box flexDirection="column" padding={1}>
      {focusList.map((item, fi) => {
        const selected = fi === cursor
        const msg = THREAD[item.msgIdx]!
        const showSep = item.msgIdx !== prevMsgIdx && prevMsgIdx !== -1
        const isNewMsg = item.msgIdx !== prevMsgIdx
        prevMsgIdx = item.msgIdx

        if (msg.role === 'user') {
          return (
            <Box key={`${item.msgIdx}`} flexDirection="column">
              {showSep && <Box><Text dimColor>│</Text></Box>}
              <Box gap={1}>
                <Text color={selected ? colors.primary : '#444444'}>●</Text>
                <Text bold={selected} dimColor={!selected}>you</Text>
        ...
        // Assistant parts
        const part = msg.parts[item.partIdx!]!
        const coll = isCollapsed(item.msgIdx, item.partIdx!)

        return (
          <Box key={`${item.msgIdx}:${item.partIdx}`} flexDirection="column">
            {/* Message header on first part */}
            {isNewMsg && (
              <>
                {showSep && <Box><Text dimColor>│</Text></Box>}
                <Box gap={1}>
                  <Text color={selected ? colors.primary : '#444444'}>○</Text>
                  <Text bold={selected} dimColor={!selected}>assistant</Text>
                  <Text dimColor>{msg.timestamp}</Text>
            ...
            <Box marginLeft={2} flexDirection="column">
              ...
              {part.type === 'tool-call' && (
                <Box flexDirection="column">
                  <Box gap={1}>
                    <Text color={selected ? colors.primary : '#555555'}>
                      {part.status === 'running' ? spin : (coll ? '▶' : '▼')}
                    </Text>
                    <Text color={part.status === 'error' ? colors.error : toolColor(part.tool, colors)} bold>
                      {part.status === 'running' ? spin : '✓'}
                    </Text>
                    <Text color={toolColor(part.tool, colors)}>{part.tool}</Text>
